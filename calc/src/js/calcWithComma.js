...
$('#btnClear').click(function(){
	...
	$('#txtHistory').val(GetComma(RemoveComma(result)));
	CalculateResult();
	if(!GetUserInput()){
		$('#txtResult').val(0);
	}
...
$('#btnPercentage').click(function(){
	var previousInput = GetPriviousInput();
	var amountToApplyPercentage = RemoveComma(previousInput);
	amountToApplyPercentage = amountToApplyPercentage.slice(0, -1);
	try{
		...
		var result = eval( amountToApplyPercentage * currentInput / 100); 
		if(result){
...
function AddDigit(digit){
	...
	var isPeriodExist = GetCurrentInput().lastIndexOf('.');
	if(isPeriodExist == -1){
		var newInput = GetComma(RemoveComma(GetCurrentInput() + digit));
	}else{
...
function CalculateResult(){
	...
	try{
		userInput = parseFloat(eval(userInput));
		$('#txtResult').val(GetComma(userInput));
	}
	catch(e){
